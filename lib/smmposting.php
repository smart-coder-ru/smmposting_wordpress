...
class Smmposting
{
     ...
     * @return bool|string
     */
    public function api($method, $body = array()) {
        ...
        $url = self::$domain."api/v$this->api_version/smmposting/$method?api_token=$this->api_token";
...
class SmmpostingWordpress
{
    ...
    /**
     * SmmpostingWordpress constructor.
     */
    ...
    public function getPost($post_id, $social = false)
    {
        switch ($social){
            case 'vkontakte':
                ...
                $sql .= "(SELECT vk_access_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.vk_account_id) as vk_access_token,
                         (SELECT vk_user_id FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.vk_account_id) as vk_user_id";
                break;
            case 'odnoklassniki':
                $sql = "SELECT p1.post_id, p1.content, p1.odnoklassniki, p2.ok_group_id,";
                $sql .= "(SELECT ok_access_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.ok_account_id) as ok_access_token";
                break;
            case 'telegram':
                $sql = "SELECT p1.post_id, p1.content, p1.telegram, p2.telegram_chat_id,";
                $sql .= "(SELECT telegram_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tg_account_id) as telegram_bot_id,
                         (SELECT telegram_chat_id FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tg_account_id) as telegram_chat_id";
                break;
            case 'instagram':
                $sql = "SELECT p1.post_id, p1.content, p1.instagram,";
                $sql .= "(SELECT instagram_login FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.ig_account_id) as instagram_login,
                         (SELECT instagram_password FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.ig_account_id) as instagram_password";
                break;
            case 'facebook':
                $sql = "SELECT p1.post_id, p1.content, p1.facebook, p2.fb_group_id,";
                $sql .= "(SELECT fb_access_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.fb_account_id) as fb_access_token,
                         (SELECT fb_user_id FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.fb_account_id) as fb_user_id";
                break;
            case 'twitter':
            ...
            case 'tumblr':
                ...
                $sql .= "(SELECT tb_oauth_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tb_account_id) as tb_oauth_token,
                         (SELECT tb_oauth_token_secret FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tb_account_id) as tb_oauth_token_secret,
                         (SELECT account_name FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tb_account_id) as account_name";
                break;
            default:
                return false;
        ...
        $sql .= ", (SELECT GROUP_CONCAT(image) FROM ".$this->wpdb->base_prefix."smmposting_image WHERE post_id = p1.post_id) as cloud_links";

        $sql .= " FROM ".$this->wpdb->base_prefix."smmposting_post p1 
                  LEFT JOIN ".$this->wpdb->base_prefix."smmposting_projects p2 ON(p1.project_id=p2.project_id) 
    ...
    public function getPosts()
    {
        ...
        $sql = "SELECT p1.post_id, p1.project_id, p1.content, p1.vkontakte, p1.date_public,p1.time_public, 
					...
					(SELECT instagram_login FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.ig_account_id) as instagram_login,
                    (SELECT instagram_password FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.ig_account_id) as instagram_password,
                    (SELECT telegram_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tg_account_id) as telegram_bot_id,
                    (SELECT telegram_chat_id FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.tg_account_id) as telegram_chat_id,
					(SELECT vk_access_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.vk_account_id) as vk_access_token,
                    (SELECT vk_user_id FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.vk_account_id) as vk_user_id,
					(SELECT ok_access_token FROM ".$this->wpdb->base_prefix."smmposting_accounts WHERE account_id = p2.ok_account_id) as ok_access_token,
					
					(SELECT GROUP_CONCAT(image) FROM ".$this->wpdb->base_prefix."smmposting_image WHERE post_id = p1.post_id) as cloud_links

					FROM ". $this->wpdb->base_prefix ."smmposting_post p1 
					LEFT JOIN ". $this->wpdb->base_prefix ."smmposting_projects p2 ON(p1.project_id=p2.project_id) 
					WHERE p1.status = 0
					AND p2.status = 1
    ...
    public function getProductsAsPosts($project_id, $products = [], $social) {
        ...
        $sql .= "(SELECT GROUP_CONCAT('".HTTP_SERVER."image/', image) FROM ".$this->wpdb->base_prefix."product_image WHERE product_id = p1.product_id) as cloud_links";

        $sql .= " FROM ".$this->wpdb->base_prefix."product p1 
                  LEFT JOIN ".$this->wpdb->base_prefix."smmposting_projects p2 ON(p2.project_id ='".$project_id."')
                  LEFT JOIN ".$this->wpdb->base_prefix."product_description p3 ON(p1.product_id = p3.product_id)
    ...
    public function updateStatus($post_id)
    {
        ...
        */
        ...
        $query = $this->db->query("SELECT DISTINCT * FROM " . $this->wpdb->base_prefix . "smmposting_post WHERE post_id = '".(int)$post_id."' ");
        $post_info = $query->row;
        ...
        */
        ...
        if ($need_to_download_post == $downloaded_post && $need_to_download_post != 0) {
            if ($errors > 0) {
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET status = 2 WHERE post_id = '".(int)$post_id."' ");
            } else {
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET status = 1 WHERE post_id = '".(int)$post_id."' ");
            }
        }
    }
    ...
    public function successSocial($post_id, $social_type, $success)
    {
        switch($social_type){
            case 'odnoklassniki':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ok_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ok_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ok_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'vkontakte':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET vk_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET vk_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET vk_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'telegram':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tg_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tg_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tg_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'instagram':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ig_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ig_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ig_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'facebook':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET fb_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET fb_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET fb_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'tumblr':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tb_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tb_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tb_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'twitter':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tw_download = 1 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tw_error = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tw_success = '".$this->db->escape($success)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            default:
                break;
        }
    ...
    public function errorSocial($post_id, $social_type, $error)
    {
        switch($social_type){
            case 'odnoklassniki':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ok_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ok_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ok_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'vkontakte':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET vk_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET vk_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET vk_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'telegram':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tg_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tg_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tg_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'instagram':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ig_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ig_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET ig_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'facebook':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET fb_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET fb_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET fb_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'tumblr':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tb_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tb_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tb_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            case 'twitter':
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tw_download = 2 WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tw_success = '".$this->db->escape(false)."' WHERE post_id = '".(int)$post_id."' ");
                $this->db->query("UPDATE " . $this->wpdb->base_prefix . "smmposting_post SET tw_error = '".$this->db->escape($error)."' WHERE post_id = '".(int)$post_id."' ");
                break;
            default:
                break;
        }
